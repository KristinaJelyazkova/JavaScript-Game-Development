...
Asteroid.prototype.child = function(mass, direction){
	var offset = this.radius / 2;
	return new Asteroid(mass, 
		this.x + direction * offset, this.y, this.x_speed, this.y_speed, this.rotation_speed);
}
...
Ship.prototype.update = function(elapsed){
	...
	this.push(this.angle + Math.PI, this.retro_on * this.retro_power, elapsed);
...
Ship.prototype.draw = function(ctx, guide){
	...
	draw_spaceship(ctx, this.radius, {guide: guide, thruster: this.thruster_on, compromised: this.compromised});
...
Indicator.prototype.draw = function(context, max, level){
	context.save();
	...
	//The maximum is shown by a stroked rectangle
	var offset = context.measureText(this.label).width;
	context.beginPath();
	context.rect(this.x + offset, this.y, this.width, this.height);
	context.stroke();
	
	//The current level is shown by a filled rectangle
	context.beginPath();
	context.rect(this.x + offset, this.y, this.width * level / max, this.height);
	context.fill();
...
NumberIndicator.prototype.draw = function(context, value){
	context.save();
